...
- [x] 4.11 Absensi Anak — tab baru "Absensi Anak" di halaman Absensi yang sudah ada (keputusan klien: tab, bukan halaman terpisah), endpoint baru `GET /api/attendance/children` berbasis `Session` (join childId/therapistId/jenis terapi via `TokenTransaction`/fallback `WeeklySchedule`) — domain terpisah dari absensi staf yang sudah ada — diverifikasi manual (nama anak/jenis terapi/terapis/jam/status tampil benar per tanggal, termasuk status "Terlaksana")
- [~] 4.12 Detail Pertemuan & Sesi Paket — quick win selesai: badge `therapyType` ditampilkan di modal "Kelola Sesi" — diverifikasi manual. Sisanya (field Kategori/Jenis Sesi baru) menunggu keputusan klien.
